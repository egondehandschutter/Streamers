package examen.streamers.data

import android.content.Context
import androidx.room.Database
import androidx.room.Room
import androidx.room.RoomDatabase

/**
 * Database class with a singleton Instance object.
 */
@Database(entities = [StreamerInfo::class], version = 1, exportSchema = false)
abstract class StreamersDatabase : RoomDatabase() {
    abstract fun streamerInfoDao(): StreamerInfoDao

    companion object {
        @Volatile
        private var Instance: StreamersDatabase? = null

        fun getDatabase(context: Context): StreamersDatabase {
            // if the Instance is not null, return it, otherwise create a new database instance.
            return Instance ?: synchronized(this) {
                Room.databaseBuilder(context, StreamersDatabase::class.java, "streamers_database")
                    /**
                     * Setting this option in your app's database builder means that Room
                     * permanently deletes all data from the tables in your database when it
                     * attempts to perform a migration with no defined migration path.
                     */
                    .fallbackToDestructiveMigration()
                    .build()
                    .also { Instance = it }
            }
        }
    }
}